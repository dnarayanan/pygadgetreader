...
{  
  ...
  /* TIPSY */
    ...
    if(type==0){  //GAS
      for(n=0;n<nselect;n++){
	fseek(infp, sizeof(float),SEEK_CUR);        //mass
	fseek(infp, sizeof(float)*MAXDIM,SEEK_CUR); //pos
	
	fread(&tmp0, sizeof(float),1,infp);
	fread(&tmp1, sizeof(float),1,infp);
	fread(&tmp2, sizeof(float),1,infp);
	
	fseek(infp, sizeof(float)*5,SEEK_CUR);      //the rest
	...
	fseek(infp, sizeof(float)*2,SEEK_CUR);      //the rest
	...
	fseek(infp, sizeof(float)*4,SEEK_CUR);      //the rest
